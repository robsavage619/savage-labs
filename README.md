...
> **Auditing the seams, not just the algorithms.**
> The worst bug I've found here wasn't a wrong formula — it was two internally-consistent catalogs that had drifted into separate namespaces. Exercise selection ranked and rotated correctly, then named a movement my logging app has never heard of, so the swap silently did nothing and I got the same lifts for months. Nothing threw. The lesson I keep relearning: a system that fails *visibly* is a solved problem, and the expensive failures are the ones where every component is individually right.
